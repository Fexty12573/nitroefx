...
static void ShowUsecase_AnimatedCounter()
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		ImGui::PushFont(ImGui::GetIO().Fonts->Fonts[0], 0.0f); // Use default font
...
static void ShowUsecase_PercentageCounter()
{
	...
	ImGui::PushFont(ImGui::GetFont(), font_scale);
...
static void ShowUsecase_WordCloud()
{
	...
	for (int i = 0; i < word_count; i++)
	{
		...
		ImGui::PushFont(nullptr, word_scale);
...
static void ShowUsecase_AnimatedTooltipText()
{
	...
	ImGui::Dummy(ImVec2(0, 10 * ImGui::GetStyle().FontScaleMain));
...
static void ShowUsecase_DamageNumber()
{
	...
	if (dmg_time >= 0)
	{
		...
		ImGui::PushFont(nullptr, pop_scale * 1.5f);
